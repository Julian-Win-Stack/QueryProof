// agent-v3 = agent-v2 minus its LIMIT sentence, which the 2026-08-01 subset
// runs measured as pure loss: it converted no target (0056's win came from the
// invented-filter clause), pushed its own target 0110 into dropping a
// justified LIMIT 1, and broke two passing controls (0179 lost the evidence's
// LIMIT 5, 0285 returned all 40 speed ties — the keep-ties shape the dead
// list's 0152 already measured as punished). Everything else is v2 verbatim —
// a prompt version freezes once a number is published under it.
