...
import { buildUserMessage, PROMPT_VERSION } from './v3.ts';
...
test('a copied prompt file does not inherit the version it was copied from', () => {
  // v3 is v1 with one rule added, so the realistic mistake is copying the file
  // and leaving the version string behind. That stamps v3 runs as v1 in every
  // export, and no later comparison recovers from it.
  assert.notEqual(PROMPT_VERSION, v1.PROMPT_VERSION);
  assert.notEqual(PROMPT_VERSION, v2.PROMPT_VERSION);
});
